...
export default function BookingPage() {
  ...
  const bgImage = 'https://images.pexels.com/photos/5993559/pexels-photo-5993559.jpeg';
  ...
  if (sent) return (
    <main style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', backgroundImage: `url(${bgImage})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
      <div style={{ textAlign: 'center', background: 'rgba(244,239,234,0.92)', padding: '40px', borderRadius: '20px', border: `3px solid ${colors.textDark}` }}>
  ...
  return (
    ...
    >
      ...
      <div style={{ backgroundColor: 'rgba(244,239,234,0.88)', minHeight: '100vh', margin: '-100px -20px -60px', padding: '100px 20px 60px' }}>
        <div style={{ maxWidth: '560px', margin: '0 auto' }} onClick={e => e.stopPropagation()}>
          <h1 style={{ fontFamily: fonts.heading, fontSize: '48px', color: colors.textDark, marginBottom: '28px', lineHeight: 1 }}>Book<br/>Appointment</h1>
          ...
          <form onSubmit={submit} style={{ display: 'grid', gap: '14px' }}>
            ...
            <div style={{ position: 'relative' }}>
              <button type="button" onClick={() => setShowServices(!showServices)} style={{...inputStyle, textAlign: 'left', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                ...
                <span style={{ transform: showServices? 'rotate(180deg)' : 'none' }}>▼</span>
              </button>
              {showServices && (
                      ...
                      {SERVICES.filter(s => s.group === group).map(s => {
                        ...
                        return (
                          <button key={s.id} type="button" onClick={() => toggleService(s.id)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: active? '#FFF4E8' : 'white', border: 'none', borderBottom: '1px solid #F0E6DD', cursor: 'pointer', textAlign: 'left' }}>
                            ...
                            <span style={{ fontFamily: fonts.body, fontSize: '15px', color: colors.textDark, fontWeight: active? 600 : 400 }}>{s.name}</span>
                          </button>
                        );
                      })}
                    </div>
                ...
                <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: '8px', backgroundColor: 'white', border: `3px solid ${colors.textDark}`, borderRadius: '16px', boxShadow: `8px 8px 0px ${colors.textDark}`, zIndex: 50, overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '12px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: '4px', marginTop: '6px' }}>
                      ...
                      {Array.from({length: daysInMonth}, (_,i) => i+1).map(day => { const d = new Date(year,month,day); const past = d < today; const sel = form.date === d.toISOString().split('T')[0]; return <button key={day} type="button" disabled={past} onClick={() => selectDate(day)} style={{ aspectRatio: '1', border: 'none', background: sel? colors.accent : 'transparent', color: past? '#ccc' : colors.textDark, borderRadius: '8px', fontWeight: sel?700:400 }}>{day}</button>})}
                    </div>
                  </div>
                </div>
              )}
